...
.card.hearts .value,
.card.diamonds .value { color:red; }

.card.clubs .value,
.card.spades .value { color:black; }
      ...
      if (card) {
        ...
        setTimeout(() => {
          card.el.style.cssText =
            "transform-origin: 50% 50%;transform: translate3d(0px,0px,0px) rotateY(0deg); transition: transform " +
