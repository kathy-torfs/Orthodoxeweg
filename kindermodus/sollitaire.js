...
(function (global) {
        ...
        if (this.siblings.length) {
          each(this.siblings, function (i, card) {
            if (
              ...
              // no extra score for moving king
            } else {
              this.score += 5;
            }
  ...
  Game.prototype.deal = function () {
    ...
    // Origineel: 3, aangepast naar telkens 1 kaart
    ...
    items.forEach((card, i) => {
      if (card) {
        ...
        card.el.style.cssText =
          `transform: translate3d(${x}px,${y}px,0px) rotateY(180deg);`;

        setTimeout(() => {
          card.el.style.cssText =
            `transform-origin: 50% 50%;
             transform: translate3d(0px,0px,0px) rotateY(0deg);
             transition: transform ${this.animationInterval}ms;`;
